...
#ID LINE : http://line.me/ti/p/~zul.1.01
# AKUHANYA PEMULA MOHON DI KEBANGIN EDIT SESUKA KALIAN ☺️☺️
# -*- coding: utf-8 -*-
import LINEPY
from LINEPY import *
...
import youtube_dl
#BOT LINE BY: TEAM TERMUX
cl = LineClient("imel@gmail.com","KATASANDI") # ganti dengan imel line kalian && kata sandi
...
ZulkifliProfile = cl.getProfile()
myProfile["displayName"] = ZulkifliProfile.displayName
myProfile["statusMessage"] = ZulkifliProfile.statusMessage
myProfile["pictureStatus"] = ZulkifliProfile.pictureStatus
...
def mentionMembers(to, mid):
    try:
        arrData = ""
        textx = "BY: TEAM TERMUX Etiket Sayısı「{}」\n\n  [ Mention ]\n1. ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def siderMembers(to, mid):
    try:
        arrData = ""
        textx = "TEAM TERMUX ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def welcomeMembers(to, mid):
    try:
        arrData = ""
        textx = "BY: TEAM TERMUX Üye Girişi「{}」\nMerhaba  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def leaveMembers(to, mid):
    try:
        arrData = ""
        textx = "BY: TEAM TERMUX Çıkan Üye「{}」\nByee  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def listharga():
    helpMessage3 = """
 Terbuka untuk menyewa tautan Shellbot 🤖
                  【 TIM TERMUX 】:

🤖 GARIS Bot Python3 :
💀 Shelbot, setengah manusia, setengah bot.
💀 Shelbot, setengah manusia, setengah bot +3 tendangan +1 hantu
💀 Shelbot, setengah manusia, setengah bot + 9 tendangan + 1 hantu

🤖 Bulan pertama 100 baht, bulan depan 50 baht.
🤖 Jaga 24 jam, jangan pernah meninggalkan pelanggan.
🤖 terputus, tersambung kembali
🤖 Shell Indonesia

🇮🇩 Kemampuan 🇮🇩
* Dapat melihat orang diam-diam membaca
* Terbuka bagi orang untuk masuk dan keluar
* Dapat menandai
* Dapat mengatur taktik
* Dapat mengatur orang untuk masuk dan keluar
* Anda dapat melihat nama dan gambar ID Anda.
* Dapat melihat pesanan, dll.
📃
📃
* 24 jam
* GARIS : zul.1.01
* Kecepatan 0,02
* Terbuka untuk pesanan Selalu sambungkan bot secara gratis.
            ...
            if wait["autoAdd"] == True:
                if op.param2 not in Bots and op.param2 not in admin:
                    if (wait["message"] in [" "," ","\n",None]):
                        ...
                        cl.sendContact(op.param1, "u35d45deb4f87d0aa580fed5649a1df80")
                    ...
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                           if msg_dict[msg_id]["text"] == 'Gambarnya dibawah':
                                ...
                                Zulkiflitj = cl.getContact(msg_dict[msg_id]["from"])
                                zx = ""
                                zxc = ""
                                zx2 = []
                                ...
                                ry = str(Zulkiflitj.displayName)
                                pesan = ''
                                pesan2 = pesan+"@x \n"
                                xlen = str(len(zxc)+len(xpesan))
                                xlen2 = str(len(zxc)+len(pesan2)+len(xpesan)-1)
                                zx = {'S':xlen, 'E':xlen2, 'M':Zulkiflitj.mid}
                                zx2.append(zx)
                                zxc += pesan2
                                ...
                                Zulkiflitj = cl.getContact(msg_dict[msg_id]["from"])
                                ret_ =  "「 Pesan Dihapus 」\n"
                                ret_ += "• Pengirim : {}".format(str(Zulkiflitj.displayName))
                ...
                try:
                    ...
                    if msg_id in msg_dict1:
                        if msg_dict1[msg_id]["from"]:
                                ginfo = cl.getGroup(at)
                                Zulkiflitj = cl.getContact(msg_dict1[msg_id]["from"])
                    ...
                    if text is None:
                        ...
                        if cmd == "help":
                          if wait["selfbot"] == True:
                               ...
                               cl.sendContact(to, "u35d45deb4f87d0aa580fed5649a1df80")
                        ...
                        if cmd == "unsend off":
                          ...
                          if msg._from in admin:
                            ...
                            if msg._from in admin:
                                try:
                                    ZulkifliProfile.displayName = str(myProfile["displayName"])
                                    ZulkifliProfile.statusMessage = str(myProfile["statusMessage"])
                                    ZulkifliProfile.pictureStatus = str(myProfile["pictureStatus"])
                                    cl.updateProfileAttribute(8, ZulkifliProfile.pictureStatus)
                                    cl.updateProfile(ZulkifliProfile)
